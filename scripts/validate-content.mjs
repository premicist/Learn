    ...
    curriculum.units.forEach((unit, unitIndex) => {
      ...
      unit.lessons.forEach((lesson, lessonIndex) => {
        ...
        if (!lesson.id || !lesson.title || !lesson.description) {
          errors.push(`${lessonLabel} needs id, title, and description`)
        }
        if (lessonIds.has(lesson.id)) errors.push(`Duplicate lesson ID in ${unitLabel}: ${lesson.id}`)
        lessonIds.add(lesson.id)
        // resourceType + resourceId are optional so curricula can be scaffolded before content exists.
        // If either is provided, both must be valid and the resource must exist.
        const hasResourceType = lesson.resourceType !== undefined && lesson.resourceType !== null && lesson.resourceType !== ''
        const hasResourceId = lesson.resourceId !== undefined && lesson.resourceId !== null && lesson.resourceId !== ''
        if (hasResourceType || hasResourceId) {
          if (!hasResourceType || !hasResourceId) {
            errors.push(`${lessonLabel} needs both resourceType and resourceId when linking a resource (or omit both to mark the lesson as coming soon)`)
          } else {
  ...
  for (const collection of MARKDOWN_COLLECTIONS) {
    for (const item of readMarkdownCollection(contentDir, collection)) {
      ...
      if (collection === 'scheduled-tests') {
        ...
        if (!Array.isArray(data.questions) || data.questions.length === 0) {
          ...
          data.questions.forEach((question, index) => {
            ...
            if (question.points !== undefined && (!Number.isInteger(question.points) || question.points < 1)) errors.push(`${questionLabel} points must be a positive integer`)
          })
        }
      }
      if (collection === 'quizzes') {
        if (!Array.isArray(data.questions) || data.questions.length === 0) {
          ...
          data.questions.forEach((question, index) => {
            const questionLabel = `${label} question ${index + 1}`
            if (!question.question) errors.push(`${questionLabel} is missing question text`)
            if (!Array.isArray(question.options) || question.options.length < 2) errors.push(`${questionLabel} needs at least two options`)
            if (!Number.isInteger(question.answerIndex) || question.answerIndex < 0 || (Array.isArray(question.options) && question.answerIndex >= question.options.length)) {
              errors.push(`${questionLabel} needs a valid answerIndex`)
            }
            if (!question.explanation) errors.push(`${questionLabel} is missing explanation`)
          })
        }
      }
      ...
      if (collection === 'videos') {
        if (!data.description) errors.push(`${label} is missing description`)
